...
<div id="hero-section"
  ...
  <?php if(!empty($idealx_hero_img_bac)){?> style="background-image: url(<?php echo esc_html($idealx_hero_img_bac) ?>);" <?php } }?>>
  <div class="hero-container uk-container">
    <div id="hero-section-content" class="hero-content">
      <?php
        if ($idealx_hero_header != '') {
            echo '<h1>' . esc_html($idealx_hero_header). ' </h1>';
        } else {
            echo '<p>
          ' . esc_html__('Edit this by going to your Dashboard -> Appearance -> Customise -> Home Hero Section', 'idealx') . '
          </p>';
        }
      ?>
      <div id="hero-section-content">
        <p><?php
          if ($idealx_hero_textarea != '') {
      ...
      <?php if(empty($idealx_hero_sec_but_enb) || $idealx_hero_sec_but_enb == 'No' ){?>
      <button id="hero-secondary" class="uk-button uk-button-secondary uk-button-large"
        onclick="window.location.href='<?php echo esc_url( $idealx_secondary_link ) ?>'"><?php
        if ($idealx_secondary_text != '') {
      ...
      ?></button>
      <?php } ?>
      <?php if(empty($idealx_hero_pri_but_enb) || $idealx_hero_pri_but_enb == 'No' ){?>
      <button id="hero-primary" class="uk-button uk-button-secondary uk-button-large"
        onclick="window.location.href='<?php echo esc_url( $idealx_primary_link ) ?>'"><?php
        if ($idealx_primary_text != '') {
